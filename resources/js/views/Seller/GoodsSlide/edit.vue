...
<script>
	export default {
	    ...
	    methods: {
	      submit:function(){
	      	var _this = this;
	      	this.$post(this.ROOT_URL + 'Admin/goods_slide/edit/' + this.$route.params.id ,{name:this.name,image_url:this.image_url,url:this.url,sort:this.sort,is_type:this.is_type}).then(function(res){
	      		_this.$message({
		          message: '恭喜你，修改成功！',
		          type: 'success'
		        });
