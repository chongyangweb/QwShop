<template>
	<div class="index_main">
		<div class="main_btn_left">
			<router-link class="admin_fff_btn" :to="{name:'seller_goods_slide_add'}"><el-button type="primary" icon="el-icon-plus">添加</el-button></router-link>

			<!-- <input class="index_search_input" type="text" placeholder="输入搜索内容"> -->
			<el-input  class="search_input" size="small" style="width:200px;" v-model="name" placeholder="请输入幻灯片标题"></el-input>
		...
		<div class="unline"></div>
			...
			<el-table ref="multipleTable" :data="lists" tooltip-effect="dark" size="medium" style="width: 100%" @selection-change="changeFun">‘
				...
				<router-link :to="{name:'seller_goods_slide_edit',params:{id:scope.row.id}}"><el-button plain icon="el-icon-edit" >编辑</el-button></router-link>
				</template>
				</el-table-column>
	...
	export default {
	    ...
	    methods: {
	      ...
	      del:function(){
	      	...
	      	this.$post(this.ROOT_URL + 'Seller/goods_slide/del',{id:this.ids}).then(function(res){
	      		_this.$message({
		          message: '恭喜你，删除成功！',
		          type: 'success'
		        });
	      ...
	      search:function(){
	      	var _this = this;
	    	_this.$post(this.ROOT_URL + 'Seller/goods_slide/index',{limit:this.page.limit,page:this.page.page,name:this.name}).then(function(res){
	      ...
	      getList:function(){
	      	var _this = this;
	    	_this.$post(this.ROOT_URL + 'Seller/goods_slide/index',{limit:this.page.limit,page:this.page.page}).then(function(res){
	    		_this.lists = res.data;
	    		_this.page = res.page;
	    	});
	      }
