<template>
	<div class="index_main">
		<div class="main_btn_left">
			...
			<el-input  class="search_input"  style="width:200px;" size="small" v-model="order_no" placeholder="请输入订单号"></el-input>
		...
		<div class="unline"></div>
			...
			<el-table ref="multipleTable" :data="lists" tooltip-effect="dark" size="medium" style="width: 100%" @selection-change="changeFun">‘
				...
				<div style="float: left;margin-right: 1rem;"><router-link :to="{path:'/Seller/order/info/'+scope.row.id}"><el-button plain icon="el-icon-edit" >查看</el-button></router-link></div>
...
<script>
	...
	export default {
	    ...
	    methods: {
	      ...
	      search:function(){
	      	var _this = this;
	    	_this.$post(this.ROOT_URL + 'Seller/order/index',{limit:this.page.limit,page:this.page.page,order_no:this.order_no,order_type:this.order_type,start_time:this.start_time,end_time:this.end_time}).then(function(res){
	      ...
	      getList:function(){
	      	var _this = this;
	    	_this.$post(this.ROOT_URL + 'Seller/order/index',{limit:this.page.limit,page:this.page.page}).then(function(res){
	    		_this.lists = res.order_list;
	    		_this.page = res.page;
	    	});
	      },
	      ...
	      add_delivery_no:function(e){
	      	...
	      	var _this = this;
	      	this.$post(this.ROOT_URL + 'Seller/order/edit_delivery_no',{order_id:this.order_id,delivery_no:this.delivery_no}).then(function(res){
	    		_this.$message({
		          message: '恭喜，设置成功！',
		          type: 'success'
		        });
