...
class BaseController extends Controller
{
    ...
    private $allowAction = ['login','checkUser','refresh']; // 后台不用验证Token的方法
    ...
    public function __construct()
    {
        ...
        // 不过刷新一次作废
        $this->middleware('jwt.auth', ['except' => $this->allowAction]);

        if(!in_array(explode('@',request()->route()->getActionName())[1],$this->allowAction)){
            // 判断是否是商户
            $user_info = JWTAuth::parseToken()->touser();
            $seller_model = new Store;
            $seller_info = $seller_model->where('id',$user_info['id'])->count();

            if(empty($seller_info) || $seller_info['status']!=1){
                return $this->errorMsg('非商家账号3！');
            }
        }
